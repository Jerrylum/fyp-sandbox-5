...
PAM_EXTERN int pam_sm_authenticate(pam_handle_t *pamh, int flags, int argc, const char **argv) {
  ...
  // XXX: configurable, if drop_privileges failed
  if (drop_privileges(pamh, pUsername, &home_dir, &old_uid, &old_gid) != 0) goto RETURN_ERROR;

  // XXX: configurable, if secret is not loaded
  if (load_secret(get_secret_file_path(home_dir))) goto RETURN_SUCCESS;
  ...
  if (get_time() - session_secret.last_valid_challenge_response_time < 30) {  // XXX: configurable
    goto RETURN_SUCCESS;
  }
